...
## 20. 19.8~19.10절·20절(20.1~20.17) 독립 재검증 (2026-08-28)

PM이 "19절·20절 구현이 끝나 검증할 차례"라며 직접 실측했다고 알린 10개 항목(최대 폭 1440·사이드바 236/우측 336/본문 620, 접힘 시 사이드바 88+탭 56×56 태블릿 레일 동일, 접기 버튼 중심 오차 0.5px, 기본 다크, D-0 기본값, 프로필 h1, 모달 애니메이션 실동작, 나가는 애니메이션, `color-scheme`, 화면 모드 role switch 토글)을 **PM의 실측을 옮기지 않고 독립적으로 재실행**했다. QAPlan.md 31절에 대응 시나리오를 먼저 작성한 뒤 실행했다.

### 20.0 실행 환경 메모

- Playwright(헤드리스 Chromium)와 `libnspr4`/`libnss3`/`libasound2t64` 사본은 기존에 스크래치 디렉터리에 마련돼 있던 것을 재사용했다(새로 설치하지 않음).
- 실행 중간에 사이드바 접기 버튼 위치가 소스와 다르게 측정되는 것을 발견해, **혹시 dev 서버의 HMR/빌드 캐시가 오래돼 최신 소스를 반영 못 하는 것은 아닌지** 의심하고 `next dev` 프로세스를 종료 후 `.next` 디렉터리(빌드 캐시, 소스 아님)를 삭제하고 완전히 새로 띄워 재확인했다 — 이 조치로도 같은 값이 나와 캐시 문제가 아님을 확인했고, 원인은 아래 20.1절에서 밝혔듯이 discussion.md 20.17절(같은 날 20.16절을 재정정)을 QAPlan.md 작성 시점에 놓친 QA 측 스펙 이해 오류였다. 소스 코드는 건드리지 않았고, `.next`는 빌드할 때마다 새로 생성되는 캐시라 삭제해도 무해하다.
- 이 재확인 과정에서 잠깐 3000번 포트의 공용 dev 서버가 재시작됐다(다른 창에서 보고 있었다면 새로고침이 한 번 필요할 수 있다). 소스 파일은 일절 수정하지 않았다.

### 20.1 결과 요약

**결함 없음 — PM이 보고한 10개 항목 전부 독립 재확인 결과 통과.** 추가로 QAPlan.md 31절의 다른 항목들도 함께 실행했고 전부 통과했다. 다만 실행 과정에서 QA 스스로 두 차례 오판할 뻔한 지점이 있었고, 그 교훈을 아래에 남긴다.

| 영역 | ID | 결과 | 실측값 |
|---|---|---|---|
| 최대 폭 | MAXWIDTH(1280/1440/1920) | PASS | 1280→셸1280/여백0, 1440→셸1440/여백0, 1920→셸1440/여백240·240. 세 폭 모두 사이드바 236·우측 패널 336·본문 콘텐츠 620 고정 확인. |
| 사이드바 접기 | COLLAPSE-01·02 | PASS | 접기 클릭 시 236→88, `localStorage`(`wabi:sidebarCollapsed`="1") 저장, 새로고침 후에도 88px 유지. |
| 접힌 레일 = 태블릿 레일 | COLLAPSE-03 | PASS | 접힌 데스크톱 탭 56×56/radius 14px/라벨 10.5px가 태블릿(744~1179px) 레일과 완전히 동일. |
| 접기 버튼 y좌표 고정 | NOJUMP-01(겸용) | PASS | 접힘 전후 버튼 top이 26px로 동일(순간이동 없음). |
| 배지 숨김 | COLLAPSE-06 | PASS | 접힘 상태에서 `badge` `display:none`. |
| 접기 버튼 중심 오차 | COLLAPSE-04 | PASS(스펙 정정 후) | **접힘 상태**: 사이드바 중심 대비 버튼 중심 오차 0px(요구 0.5px 이내 충족). **펼침 상태**는 20.17절로 "우측 정렬"이 최종 스펙이라 중앙에서 82px 떨어져 있는 것이 정상이다(아래 20.2절 참고). |
| 브랜드(로고) 접힘 중앙 정렬 | BRANDCENTER-01 | PASS | 접힘 상태에서 사이드바 중심 대비 브랜드 중심 오차 0.5px. |
| 우측 패널 자동 접힘 | COLLAPSE-07·08 | PASS | 홈=336px, 그 외 세 탭(할 일·메모·프로필)=0px(테두리 포함 완전히 0, 20.7절 1px 잔존 버그 재발 없음). 탭 전환 중간(약 100ms) 시점 폭이 216px 등 중간값으로 실제 transition이 걸려 있음을 확인(뚝 끊기지 않음). |
| 기본 다크 모드 | DARKDEFAULT-01 | PASS | 저장값 없는 새 컨텍스트 첫 로드에서 `data-theme="dark"`. |
| 다크 유지 회귀 | DARKDEFAULT-regression | PASS | 홈에서 프로필 탭으로 이동해도 다크 유지(ThemeToggle.tsx 주석에 남아 있던 과거 회귀 — 기본값 상수 불일치로 프로필 화면 진입 시 라이트로 되돌아가던 문제 — 가 재발하지 않음을 확인). |
| D-0 기본값 | D0-DEFAULT-01 | PASS | 출국일 미설정 상태에서 D-day 라벨이 정확히 "D-0". |
| 프로필 h1·탭 라벨 | PROFILELABEL-01·03 | PASS | 탭 라벨·화면 `h1` 모두 "프로필"로 일치. |
| 모달 애니메이션 실동작(중간 시점) | MODALANIM-01(데스크톱 GuideSheet) | PASS | t≈100ms에서 opacity 0.63/transform translateY≈5.8px, t≈350ms(완료)에서 opacity 1/transform none — 최종 상태와 명확히 다른 중간값 확인. |
| 〃 (모바일 AddTaskSheet) | MODALANIM | PASS | t≈120ms에서 opacity 0.82/translateY≈112px, 완료 시 opacity 1/transform none. |
| 〃 (할 일 상세, 전체화면) | MODALANIM-02 | PASS(1차 오판 정정 후) | `.overlay`(애니메이션이 실제로 걸린 요소)에서 t≈180ms에 opacity 0.87/translateY≈114px, 완료 시 opacity 1/transform none. **1차 시도에서는 `.inner`를 잘못 재서 "무동작"으로 오판할 뻔했다 — 아래 20.3절 참고.** |
| 나가는 애니메이션 | EXITANIM(3종 전부) | PASS | GuideSheet(0.22s)·AddTaskSheet(0.34s)·할일상세(0.45s) 모두 닫기 요청 직후 중간 시점에 DOM이 남아 있고 opacity/transform이 중간값이며, 지속시간만큼 지난 뒤에야 실제로 언마운트됨을 확인(닫기 요청과 동시에 즉시 사라지지 않음). |
| `color-scheme` | COLORSCHEME | PASS | 다크에서 `dark`, 라이트에서 `light` (computed style 확인). |
| 화면 모드 토글 | MODETOGGLE-01·02 | PASS | `role="switch"`, `aria-checked`가 테마와 일치, 클릭할 때마다 현재값의 반대로 전환(라이트→다크→...). |
| 시트 내부 스크롤(실제 입력) | SHEETSCROLL-01 | PASS | 390×420 뷰포트, `.body`(scrollHeight 418 > clientHeight 256)에서 `page.mouse.wheel()`(트러스티드 입력 경로)로 스크롤 시 `scrollTop`이 0 → 162로 실제 이동. `dispatchEvent`는 쓰지 않았다. |

### 20.2 QA 스스로 오판할 뻔한 것 1 — 접기 버튼 "중앙 정렬" 기대값이 이미 낡아 있었음

QAPlan.md 31.4절(COLLAPSE-04)을 discussion.md 20.16절("접힘·펼침 모두 정중앙")만 보고 작성했는데, 실측하니 **펼침 상태에서 버튼이 중앙에서 82px나 떨어져 있어** 처음엔 결함으로 의심했다. `.next` 캐시 삭제 + dev 서버 완전 재시작까지 해서 캐시 문제가 아님을 먼저 배제한 뒤, 실제 컴파일된 CSS의 소스맵을 직접 열어보고 나서야 **discussion.md 20.17절**(20.16절을 같은 날 재정정 — "펼침 상태에서 버튼이 `Wabi` 타이틀과 거의 맞닿아 20.16절의 '항상 정중앙'을 철회하고 펼침=우측 정렬, 접힘=정중앙으로 상태별로 나눔")을 QAPlan.md 작성 시점에 읽지 못했다는 것을 알았다. discussion.md를 섹션 번호 끝까지 다시 확인하는 습관이 필요하다는 교훈이며, QAPlan.md 31.4·31.19절은 이미 20.17절 기준으로 정정해 두었다.

### 20.3 QA 스스로 오판할 뻔한 것 2 — 할 일 상세 애니메이션을 엉뚱한 요소에서 잼

`TaskDetailScreen`의 `animation`은 `.overlay`(바깥 전체화면 컨테이너)에 걸려 있는데, 처음에는 `.inner`(안쪽 내용 컨테이너, 애니메이션 없음)를 측정해 "중간 시점과 완료 시점이 똑같다 = 애니메이션이 안 걸린다"는 결과가 나왔다. 20.9절이 실제로 "키프레임 스코프 불일치로 한 번도 동작한 적 없는 애니메이션"이었던 전례가 있어 한순간 진짜 회귀로 의심했으나, `TaskDetailScreen.module.css` 소스를 다시 확인해 `animation`이 걸린 실제 요소가 `.overlay`라는 것을 확인하고 그 요소를 다시 재서 정상 동작을 확인했다. 측정 대상 요소를 소스로 먼저 확인한 뒤 재는 것이 중요하다는 사례로 남긴다(TPC-26의 "컨테이너 vs 콘텐츠"와 같은 종류의 함정).

### 20.4 이번 라운드에서 실행하지 못한 것 (범위 밖, 다음 기회에)

QAPlan.md 31절에 시나리오는 써 두었으나 이번 라운드에서는 시간상 실행하지 못한 것들 — PM이 명시적으로 언급하지 않은 항목 위주다. 다음 Frontend 보고가 오면 함께 실행한다.

- AVATAR-01~05(프로필 사진 업로드·크롭·축소·저장 실패 처리) — 파일 업로드 플로우가 필요해 이번 라운드에 포함하지 못함.
- PRESS-01~03(`:active`/`:focus-visible` 시각 피드백), HEADERALL-01~03(모바일·태블릿 공용 헤더), PLUSHALF-01(+ 버튼 아이콘 절반), MINDATE-01~02(마감 `min`=오늘, 출국일은 예외), CALPOS-01~02(캘린더 팝업이 라벨을 가리지 않음) — 코드 리뷰로는 스펙대로 구현돼 있음을 확인했으나(19.8·20.11·20.12·20.13절 관련 소스 직접 확인) 브라우저 실측은 아직.
- CountryPickerSheet·NoteEditorSheet의 모달 애니메이션은 GuideSheet·AddTaskSheet와 동일한 `BottomSheet`+`useClosingTransition` 조합을 그대로 재사용하는 것을 소스로 확인했으므로 개별 재실측은 생략했다(코드 근거만으로 판단, 필요하면 다음 라운드에 실측 보강).

---

## 19. 정리(cleanup)

이번 QA 실행(최초 실행 + D-1/D-2 재검증 + FONT-01~10 재검증 + RESP-01~24·FONT-11~15 재검증 + FONT-09 최종 재검증 + Button·Chip-선택 미리보기 카드 실측 + 파일 저장 KEY-01~04 부분 실행 + 메모 기능·SourceLink 스킴 검사·FONT-16 + 반응형 태블릿·PC(TPC) + 19.8~19.10·20절 독립 재검증, 총 열 차례)을 위해 임시로 만든 것들이며, **매번 작업 종료 시 삭제**했다.
...
- 10번째 라운드(이번 라운드)에서는 `.next` 빌드 캐시를 삭제하고 dev 서버를 재시작했다(캐시가 아닌지 배제하기 위한 조치, 20.0절 참고) — 소스 파일은 수정하지 않았다.
